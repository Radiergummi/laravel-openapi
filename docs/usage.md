...
**Spec looks wrong / missing — first check the generation log.** During
`php artisan openapi:generate`, extractors emit warnings for anything they couldn't introspect:

- `[OpenAPI] Response schema introspection failed for …` — the primary-response resolver couldn't
  resolve a return type. Add `#[ResponseResource(YourResource::class)]`.
- `[OpenAPI] Schema introspection failed for FormRequest …` — the FormRequest's `rules()` calls
  `auth()->user()` or another runtime-only helper. The body schema falls back to a bare object.
- `[OpenAPI] Skipping validation rule extraction for …Data: …` — same story for a Data class. The
  schema falls back to type-only (no validation constraints), but the endpoint still appears.

**A new endpoint isn't appearing.** Confirm the route is actually registered
(`php artisan route:list`) and not excluded by a configured route filter.

**Two Data classes with the same basename collide.** `ComponentSchemaRegistry` disambiguates by
prepending the parent namespace segment (e.g. `Projects.CreateProjectData`). If you see a strange
compound name in `components.schemas`, that's why.

**A custom `Rule` object yields no constraint.** Use the schema-transformer hook to inject
constraints — see [Schema transformer](#schema-transformer).

**`#[RequestField]` on a URI parameter, or `#[PathParam]` on a Data property.** The
`field.attribute-wrong-scope` lint rule (SpatieData plugin, level 1) catches this. Use
`#[PathParam]` for route-bound parameters and `#[RequestField]` for request-body properties.
