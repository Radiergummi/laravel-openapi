...
| `QueryParam` | class, method | yes | Document an ad-hoc query string parameter, or override an inferred `$request->query()` read of the same name. Each instance defines one parameter. |
...
| `ResponseResource` | class, method | no | Explicit response-resource class, for a return type too loose to name one. `collection: true/false` overrides envelope detection; `null` auto-detects. The attribute names the class and cardinality, not the status: the resource is documented under the status a `response()->json(<resource>, <status>)` wrapper authors when that is statically readable, and otherwise under the route's conventional success status (`200`, or `201` on a resourceful `store()`). A status that may carry no body — a non-2xx, a `204`, or a `205` — makes the response yield entirely. |
...
| `Header` | class, method | yes | Document a custom request header parameter on the operation, or override an inferred `$request->header('…')` read of the same name. Header names are matched case-insensitively (RFC 9110), so a case-differing attribute and inferred read collapse to one parameter; cookie names stay case-sensitive. |
| `CookieParam` | class, method | yes | Document a cookie parameter, or override an inferred `$request->cookie('…')` read of the same name. Each instance defines one `in: cookie` parameter; the schema type defaults to `string`. A bare `$request->cookie('…')` read is already inferred (`string`, optional) — reach for this attribute to add a type, description, `required`, enum, or other constraints. |
