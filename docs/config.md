...
| `security_middleware_map` | Map of custom guard-middleware name → a scheme declared in `security_schemes`. A route carrying a mapped middleware emits that scheme's per-operation requirement (alongside the auto-derived `auth:*`/`scope:*` handling), so project-specific guards don't make a protected endpoint look public. |
