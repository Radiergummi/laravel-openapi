...
| `read_migration_columns` | Default `true`. When enabled, Eloquent model response fields are enriched with column signals read statically from `database/migrations/*.php` (formats, `maxLength`, unsigned `minimum`, decimal `multipleOf`, enum members, nullability, defaults, `comment`→`description`), ranking below `$casts`/`@property`/attributes. Set to `false` to skip migration parsing. See [Eloquent model response schemas](auto-derivation.md#eloquent-model-response-schemas). |
