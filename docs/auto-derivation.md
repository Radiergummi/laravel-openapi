...
| Response headers | A `201` response → a `Location` header (`string`, `uri-reference`), the URL of the created resource — skipped when the body is positively scalar (every declared media type carries a `string`/`integer`/`number`/`boolean` schema, as a streamed download does), since nothing addressable was created; a content-less `201`, or one whose schema declares no readable type, keeps it. `throttle` middleware (route-declared or controller-applied, see [Controller middleware](#controller-middleware)) → `X-RateLimit-Limit` and `X-RateLimit-Remaining` (`integer`) on the success response. An authored `#[ResponseHeader]` of the same name on the same status always wins. |
