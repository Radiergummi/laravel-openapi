...
| Response body | Spatie Data class or `DataCollection<…>` return type → component `$ref`. `JsonResource` subclass → component schema: fields are inferred from a single-`return [...]` `toArray()` literal (or a variable assigned one such literal once, unconditionally) (`$this->field` typed from the wrapped `@mixin`/`@extends` model or value object, `->format(…)` on a date receiver typed from either source, nested resources as `$ref`s, `when*`/`unless` wrappers optional) composed with declared `#[ResourceField]` attributes, which win per field; a passthrough or dynamic `toArray()` falls back to the wrapped model's schema (see [Plugins → ApiResources](plugins.md#apiresources)). A **base** resource return type (`JsonResource`, bare `ResourceCollection`, `AnonymousResourceCollection`) — or a **loose response wrapper** too generic to name a payload (`JsonResponse`, `Response`, and their Symfony parents) — resolves the concrete resource from the method's return expression — `X::collection(…)` / `X::make(…)` / `new X(…)`, `->toResource(…)`, a resource wrapped in a direct `response()->json(<resource>, <status>)` (whose statically-readable **body-bearing 2xx** status — literal or class constant, positional or named — becomes the response status and wins over the resourceful-route convention; a `204` is claimed by the inline-JSON resolver as a body-less response instead — except on an action carrying `#[ResponseResource]`, which that resolver skips, so the route convention supplies the status there — and any status that may carry no content — a **non-2xx**, a `204`, or a `205` — makes the resource path yield entirely, leaving the authored error status to the error subsystem. The wrapper status is read on the `#[ResponseResource]` path too: the attribute names the resource class and cardinality, not the status), or a `@return …Collection<X>` generic, or multiple returns that all resolve to the same resource (bare `return;` / `return null;` guards ignored); a collection only claims the paginated envelope when its source visibly ends in a `paginate()`-family call, looking through paginator-preserving links like `withQueryString()` (bounded scan; see [Plugins → ApiResources → Resolving the resource from the return expression](plugins.md#resolving-the-resource-from-the-return-expression)). Eloquent `Model` subclass → component schema built from `$casts`, `@property`/`@property-read` annotations, typed `$appends` accessors, and `$hidden`/`$visible`; the same schema is recovered from a directly-returned `Model::find()`/`findOrFail()`/`firstOrFail()` call on an untyped action (bounded scan). See [Eloquent model response schemas](#eloquent-model-response-schemas). Without a schema-bearing return type, a literal `response()->json([...])` or `new JsonResponse([...], status)` construction in the method body is read instead (bounded scan; see [Inline JSON responses](#inline-json-responses)). A non-paginator return type whose body unconditionally calls `paginate()`/`simplePaginate()`/`cursorPaginate()` gets the matching paginated envelope when an item class is declared (`#[ResponseResource(Model::class)]` or a `@return Paginator<Item>` generic); this defers to API Resources / Spatie Data whenever the return type or a resource-naming `#[ResponseResource]` is one of theirs. With the Fractal plugin enabled, a transformer's schema is inferred from its single-`return [...]` `transform()` literal (or a variable assigned one such literal once, unconditionally) (`$model->field` typed from the typed parameter, casts by their JSON type) composed with `#[TransformerField]` attributes, which win per field; the `$entity_transformer` + `itemResponse()`/`listResponse()` base-controller convention binds it without an attribute (see [Plugins → Fractal](plugins.md#fractal)). |
| Response headers | A `201` response → a `Location` header (`string`, `uri-reference`), the URL of the created resource — skipped when the body is positively scalar (every declared media type carries a `string`/`integer`/`number`/`boolean` schema, as a streamed download does), since nothing addressable was created. A nullable scalar counts as scalar in either spelling, so `?string` (`type: ['string', 'null']`) is skipped too; a type array with a non-`null` member that is not scalar (`['object', 'null']`) keeps the header, as do a content-less `201`, a media type with no schema, and a schema declaring no readable type. `throttle` middleware (route-declared or controller-applied, see [Controller middleware](#controller-middleware)) → `X-RateLimit-Limit` and `X-RateLimit-Remaining` (`integer`) on the success response. An authored `#[ResponseHeader]` of the same name on the same status always wins. |
...
- **`RedirectResponse`** — a `302 Found` with a `Location` response header
  (`type: string, format: uri`) and no body. The type constrains the status to a
  redirect status (Symfony rejects anything outside
  `{201, 301, 302, 303, 307, 308}`), so the conventional `200`/`204` cannot be
  right: this status is treated as read from the action, and
  `store()`/`update()`/`destroy()` actions keep the `302` and its `Location`
  instead of being rewritten to `201`/`200`/`204`. `302` is what `redirect()`
  produces; an action that redirects with a different status should say so with
  `#[Response]`, which wins as always. (The one case this trades away is a
  `store()` returning `new RedirectResponse($url, 201)`, which the convention
  previously got right by accident.)
- **`StreamedResponse` / `BinaryFileResponse`** — a binary `200`
  (`application/octet-stream`, `{type: string, format: binary}`). Only the media
  type is derived from the return here; the status is unconstrained (a streamed
  response accepts any), so the route convention may still promote it
  (`store()` → `201`). A `destroy()` keeps the binary `200` rather than the
  conventional `204`, which forbids a body.
...
8. **Keyword applicability** — once every source above has contributed, a keyword that constrains only one JSON Schema type class is dropped when the resolved type retains no member of that class: numeric bounds (`minimum`, `maximum`, `exclusiveMinimum`, `exclusiveMaximum`, `multipleOf`) off a non-numeric type, string bounds (`minLength`, `maxLength`, `pattern`) off a non-string type, and array bounds (`minItems`, `maxItems`, `uniqueItems`) off a non-array type. A union keeps a keyword as long as one member of its class remains (`['string', 'null']` keeps `maxLength`; `['integer', 'string']` keeps both families), and an untyped property (step 7) keeps all of them, since it constrains whatever instance arrives. Each discarded keyword is reported in the generation log as a warning, because it means the model and its migration disagree about that column: a model that migrated its key to a ULID (`@property string $id`) over an old `increments('id')` migration would otherwise document `{"type": "string", "minimum": 0}`.

   Two keywords are checked the same way but on their own terms, since neither is scoped to a single type class. **`format`** is defined per format, so it is dropped when the resolved type has no member the format applies to — `@property int $published_on` over a `date('published_on')` column documents a plain `integer` rather than `{"type": "integer", "format": "date"}`, which validators ignore but client generators read as a date. A format the generator does not itself emit (a custom or newly registered one) is always kept, never pruned. **`enum`** is checked against its members' own types: it is dropped only when *no* member could match the resolved type, so `@property int $status` over an `enum('status', ['draft', 'published'])` column loses the members it could never satisfy. This is the more serious of the two, because `enum` applies whatever the `type` says: `{"type": "integer", "enum": ["draft", "published"]}` matches nothing at all, failing every integer on the `enum` and every string on the `type`. One matching member keeps the whole `enum` (a nullable column's `['string', 'null']` keeps its string members), and individual members are never filtered out.
...
- A **schema-bearing return type** (a Model, Data class, Resource, or
  ...
  `#[FractalResponse]` — is never scanned **for a literal body**, even though the
  resolver consuming the attribute runs later: explicit authoring always wins.
  `#[ResponseResource]` does have a read of its own, for the wrapper's *status*
  only (see [Plugins → ApiResources](plugins.md#resolving-the-resource-from-the-return-expression));
  `#[FractalResponse]` reads neither.
