...
- `openapi:lint --no-spec-validation`: skips the OAS 3.1 meta-schema validation (the `spec.invalid` rule), which runs on every lint and is the single most expensive rule on large specs (~10%+ of lint self-time via `opis/json-schema`). The rule stays **on by default** and remains non-suppressible via `#[IgnoreLint]` / `--skip` / `disabled_rules`; the new flag is a dedicated, explicit opt-out for perf-sensitive runs. Companion to `openapi:generate --no-validate` (#124). See [Linting](docs/linting.md). (#125)
