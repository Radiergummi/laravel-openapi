...
- **`openapi:lint --migrate`** — migration mode for apps moving off hand-authored swagger-php annotations onto inference. The new `migration.oa-redundant-with-inference` rule (registered by the off-by-default `SwaggerPhp` plugin) flags a class whose authored `#[OA\Schema]` / `@OA\Schema` the generator already reproduces, and `--fix` removes the whole `#[OA\Schema]`+`#[OA\Property]` set or the `@OA\Schema` docblock block. The decision is **provenance-based, not name-based**: the class's authored schema is compared — by source class, ignoring serialized names — against what inference produces for the same class in a second, inference-only generation (a new `InferenceControlDocument`, built by running the pipeline with the harvester stage excluded via `SpecPipeline::withoutStage()`). It fires only when inference **subsumes** the annotation — reproduces everything it said and possibly more (a synthesised example, a discovered property) — so a `description` or `additionalProperties: false` inference can't derive keeps the annotation load-bearing; and never when another surviving authored annotation still `$ref`s the schema, so the fix can't dangle a reference. "Same generated document" means same API surface: removal may consolidate a component to inference's name. Migration rules trigger that second generation, so they stay inert (and free) unless `--migrate` is passed. See [Linting → Migration mode](docs/linting.md#migration-mode---migrate). (#122)
