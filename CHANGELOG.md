...
- Regression guard: `#[RawSchema]` documents using the nested-schema keywords `additionalProperties` (as a schema), `patternProperties`, `propertyNames`, `contains`, and `discriminator` are asserted to pass swagger-php's `validate()` on both supported majors (5.8 and 6.x). These keywords serialise correctly as raw arrays on both legs, so no conversion is needed. (#352)
