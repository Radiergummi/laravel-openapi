...
- The OpenAPI stage pipeline order is now expressed as a single ordered sequence in one place — the `OpenApiServiceProvider` registry factory closure — instead of being split across `BaselineRegistration` (baseline stages first), the plugin loop, and two terminal steps `SpecPipeline` held as injected fields. The closure now reads top-to-bottom: pre-plugin baseline stages (`RootStage` → `PathsStage` → `ErrorResponseInferenceStage`) → plugin stages → post-plugin stages (`ComponentsStage` → `SecurityStage` → `OverridesStage` → `TransformersStage`). `SpecPipeline` becomes a pure executor that just loops the registered stages. The `ComponentsStage` flush consequently runs **after** the plugin loop, so a late stage that contributes schemas (the SwaggerPhp harvester, and any future one) registers them on `ComponentSchemaRegistry` like every other contributor — the harvester's bespoke direct-write to `$document->components` (with its own dedup, no schema-transformer dispatch) is removed in favour of `registerNamed()`. The terminal-stage "runs last" guarantee is now positional (it holds because all registration funnels through the factory closure before the terminal `addStage` calls) rather than structural; `OpenApiRegistry::seal()` is called at the end of the closure to enforce that funnel, rejecting out-of-band `addX()` on the built registry with an (unchecked) `RegistrySealedException`. Internal wiring only — no change to the generated document for any bundled configuration. (#190)
