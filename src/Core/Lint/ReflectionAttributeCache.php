 ...
 * `$reflector->getAttributes()` once per reflector and buckets the result by attribute FQCN —
 * every caller that asks for a specific attribute class then reads from the bucket rather than
 * walking the attribute list again.
 *
 * Sibling lint rules that all key off the same target attribute (e.g. the three Fractal rules
 * that read `#[FractalResponse]` off the same method, or the two resource rules that walk
 * `#[ResourceField]` on the same resource class) share a single bucket build through this cache.
 *
 * Construction of `ReflectionClass` from a class-string is also memoised, so sibling rules can
 * pass a class FQCN and share the same {@see ReflectionClass} instance.
 *
...
final class ReflectionAttributeCache
{
    ...
    public function attributes(
        ...
        return $this->bucketFor($reflector)[$attribute] ?? []; // @phpstan-ignore return.type (bucket keyed by $attribute holds matching ReflectionAttribute<T> by construction)
    }
