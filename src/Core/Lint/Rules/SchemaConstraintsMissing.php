...
/**
 * Reports schemas that declare a broad type but omit the constraint that
 * bounds it: strings without `maxLength`, arrays without `maxItems`, and
 * integers/numbers without `minimum` or `maximum`.
 *
 * This rule is intentionally noisy — it fires on every unconstrained field —
 * and is therefore shipped DISABLED by default (see config/openapi.php
 * `lint.disabled_rules`). Teams that want to enforce constraint coverage can
 * opt in explicitly.
 *
 * Exemptions:
