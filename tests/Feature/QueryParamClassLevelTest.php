...
it(
    'lets method-level #[QueryParam] override the class-level entry with the same name and append new ones',
    function (): void {
        $params = queryParameters($this->spec['paths']['/oa-fixture/query-param/override']['get']['parameters'] ?? []);

        expect($params)
            ->toHaveKeys(['tenant', 'locale', 'page'])
