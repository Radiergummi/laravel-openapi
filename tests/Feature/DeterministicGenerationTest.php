...
/** @return list<string> Every operationId emitted across the document, in document order. */
function collectOperationIds(array $spec): array
{
    $ids = [];
...
it('emits a unique operationId for every operation', function (): void {
...
it('produces identical output across two independent generation runs', function (): void {
    $first = generateSpec();
    ...
    expect($second)->toEqual($first)
        ->and(collectOperationIds($second))->toBe(collectOperationIds($first));
});
