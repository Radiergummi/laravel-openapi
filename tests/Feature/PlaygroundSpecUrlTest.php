...
it('falls back to the route-derived spec URL when spec_url is empty for Swagger UI', function (): void {
    config([
        'openapi.routes.playground.renderer' => 'swagger-ui',
        'openapi.routes.playground.spec_url' => '',
    ]);

    $response = $this->get('/api/docs')->assertOk();

    // A blank env var must not produce an empty url; the route-derived URL is used instead.
    expect($response->getContent())
        ->toContain(route('openapi.spec'))
        ->not->toContain('url: ""');
});

it('falls back to the route-derived spec URL when spec_url is whitespace-only', function (): void {
    config(['openapi.routes.playground.spec_url' => '   ']);

    $response = $this->get('/api/docs')->assertOk();

    // blank('   ') is true, so a whitespace-only value is treated as unset.
    expect($response->getContent())
        ->toContain(route('openapi.spec'))
        ->not->toContain('data-url="   "');
});
