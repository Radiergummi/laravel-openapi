...
it('builds each inline branch as its own object component', function (): void {
    ...
    $awsRef = $spec['components']['schemas'][substr(
        (string) discriminatedBodySchema($spec, '/oa-139/inline')['discriminator']['mapping']['aws'],
        strlen('#/components/schemas/'),
    )];
...
it('resolves a class-string branch through the ref chain', function (): void {
    ...
    expect($schema['discriminator']['mapping']['custom'])
...
it('auto-injects the discriminator property as a single-value enum string', function (): void {
    ...
    expect($branchA['properties']['type']['type'])
        ->toBe('string')
...
it('lets an authored discriminator field win over injection', function (): void {
    ...
    expect($branchB['properties']['type']['description'])
        ->toBe('Authored discriminator.')
...
it('resolves multiple class-string branches through the ref chain', function (): void {
    ...
    expect($schema['discriminator']['mapping'])
        ->toBe([
...
it('resolves a class-string #[RequestField] type inside an inline branch to a $ref', function (): void {
    ...
    expect($branch['properties']['detail']['$ref'])
        ->toBe('#/components/schemas/CircleData')
        ->and($spec['components']['schemas'])->toHaveKey('CircleData');
})->group('plugin:spatie-data');
