...
it('includes data-configuration in the Scalar shell when preferred_scheme is set', function (): void {
    config(['openapi.routes.playground.auth.preferred_scheme' => 'bearer']);

    expect($this->get('/api/docs')->assertOk()->getContent())
        ->toContain('data-configuration')
        ->toContain('preferredSecurityScheme');
});

it('omits data-configuration from the Scalar shell when preferred_scheme is null', function (): void {
...
it('omits data-configuration from the Scalar shell when auth config is absent', function (): void {
    // No auth config set at all — the page must still load and omit data-configuration.
    expect($this->get('/api/docs')->assertOk()->getContent())
        ->not->toContain('data-configuration');
});

it('HTML-encodes the preferred_scheme value in the Scalar data-configuration attribute', function (): void {
